...
class OfflineFirstSyncRepository(
    ...
    private suspend fun backfillNewAndPendingGroups(
        ...
        (newlyKnownGroupIds + pendingBackfillStore.getAll())
            .toSet()
            .forEach { groupId ->
                tabEntryBackfiller.backfill(groupId)
                // Schedules have the same gap as entries: the delta filters them by the cursor, so
                // a group that just became visible arrives without the ones it already had.
                recurringSeriesRepository.refreshSeriesForGroup(groupId)
            }
    }
    ...
    private suspend fun applySnapshot(
        ...
        // Series before entries: a generated entry names its series, and the projector reads both.
        // Only a full sync may prune — a delta says nothing about the series it omits, and a series
        // is never deleted server-side, only deactivated.
        recurringSeriesLocalWriter.persist(
            series = snapshot.recurringSeries,
            namedParticipants = snapshot.referencedParticipants,
            staleSeriesIds =
                if (isFullSync) {
                    val serverIds = snapshot.recurringSeries.mapTo(mutableSetOf()) { it.seriesId }
                    database.recurringSeriesDao.getAllSeriesIds().filterNot { it in serverIds }
                } else {
                    emptyList()
                },
        )

        // Recorded from alive and soft-deleted entries alike, and before the merge below hard-
        // deletes the latter. A claimed slot is never regenerated by the server, so losing the
        // record is what would make a deliberately deleted occurrence reappear as a placeholder.
        recordRecurringSlotClaims(snapshot.tabEntries)
        ...
        // The table carries no foreign key — a claim can arrive before its series does — so nothing
        // else removes the rows of a group the user has left.
        database.recurringSlotClaimDao.deleteClaimsForRemovedGroups()
    }

    private suspend fun recordRecurringSlotClaims(entries: List<TabEntry>) {
        val claims =
            entries.mapNotNull { entry ->
                val seriesId = entry.recurringSeriesId ?: return@mapNotNull null
                val occurrenceDate = entry.recurringOccurrenceDate ?: return@mapNotNull null
                RecurringSlotClaimEntity(
                    seriesId = seriesId,
        ...
        if (claims.isNotEmpty()) {
            database.recurringSlotClaimDao.recordClaims(claims)
        }
    }
}
